...
<script src="js/validacao-rubricas.js"></script>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.2.0/js/bootstrap-datepicker.min.js"></script> 
   <script>
    // CÓDIGO COMPLETO DIRETO NO HTML
    $(document).ready(function() {
        console.log('jQuery funcionando:', typeof $);
        ...
        function validarRubrica(campo, exclusoes) {
            var valor = $(campo).val().trim();
            if (valor === '') return;
            
            if (exclusoes && exclusoes.includes(valor)) {
                alert('Código não permitido: ' + valor);
                $(campo).val('').focus();
            }
        }
        ...
        setTimeout(function() {
            // Servidor Civil (sem exclusões)
            $('#servidor7').on('blur.validacao', function() {
                console.log('Validando servidor civil:', this.value);
                validarRubrica(this, []);
            });
            
            // Militar Ativo (com exclusões)
            $('#ativa10').on('blur.validacao', function() {
                console.log('Validando militar ativo:', this.value);
                validarRubrica(this, exclusoesCompletas);
            });
            
            // Pensionista (com exclusões)
            $('#pensionista10').on('blur.validacao', function() {
                console.log('Validando pensionista:', this.value);
                validarRubrica(this, exclusoesCompletas);
            });
            
            // Veterano (exclusão mínima)
            $('#VeteranoAnistiado10').on('blur.validacao', function() {
                console.log('Validando veterano:', this.value);
                validarRubrica(this, ["1200001 - AC CONTAS"]);
            });
            
            // Qualquer campo com "Rubrica" no placeholder
            $('[placeholder*="Rubrica"], [placeholder*="rubrica"]').on('blur.validacao', function() {
                console.log('Validando campo genérico:', this.id, this.value);
                validarRubrica(this, exclusoesCompletas);
            });
            
            console.log('Validação aplicada em todos os campos!');
        }, 3000); // Aguarda 3 segundos para campos carregarem
    });
    </script>
...
<div id="menuTotalFixo">
  ...
  <h1 style="color:#7F7F7F; font-family: 'Open Sans'; margin:0;">PagTesouro PAPEM</h1>
  <a href="manualPagTesouro.pdf" target="_blank" class="btn btn-outline-secondary btn-sm">
    <i class="fa fa-book"></i> Manual do PagTesouro
  </a>
</div>
